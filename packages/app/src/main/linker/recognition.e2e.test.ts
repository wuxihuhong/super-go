...
describe.skipIf(!existsSync(MODEL_PATH))('识别端到端（真模型 + 截图 fixture）', () => {
  it('标准平台风格截图（测试靶）识别出标准初始局面', { timeout: 60_000 }, async () => {
    ...
    const rec = recognizeFrame(await detectFixture('screen-initial.png'));
    if (!rec.ok) throw new Error(`recognizeFrame ${rec.kind}`);
  ...
  it('TCHESS 真实平台截图：识别出截图中的实际局面（炮八平五 马2进3 后）', { timeout: 60_000 }, async () => {
    ...
    const rec = recognizeFrame(await detectFixture('tchess-init.png'));
    if (!rec.ok) throw new Error(`recognizeFrame ${rec.kind}`);
    const frame = rec.frame;
