...
export class YoloSession {
  ...
  async detect(img: RawImage): Promise<{ detections: Detection[]; inferMs: number; peakScore: number }> {
    ...
    if (output === undefined) return { detections: [], inferMs, peakScore: 0 };
    const data = output.data as Float32Array;
    // 输出 dims = [1, 4+nc, anchors]；anchors 从 dims 取（不假设 8400）
    const anchors = Number(output.dims[2] ?? Math.floor(data.length / 19));
    const decoded = decodeDetections(data, anchors, lb, img.width, img.height);
    return { detections: nmsByClass(decoded.detections), inferMs, peakScore: decoded.peakScore };
  }
